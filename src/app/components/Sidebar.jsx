...
const Sidebar = ({ setSideOpen, sideOpen }) => {
    ...
    useEffect(() => {
        ...
        const handleClickOutside = (event) => {
            if (sidebarRef.current && !sidebarRef.current.contains(event.target)) {
                setSideOpen(false); // Close the sidebar
            }
        };
    ...
    return (
        <div ref={sidebarRef} className={`sidebar fixed bg-slate-300 w-96 h-full top-0 z-50 p-6 duration-500 ${right} `} >
            ...
            <div className="links flex flex-col gap-12 items-center mt-24">
                ...
                <SideLink href='/skills'>Skills</SideLink>
                <SideLink href='/about'>About</SideLink>
...
function SideLink({ children, href = "/"}) {
    return <Link href={href} className="block text-white text-4xl mt-4 hover:translate-x-3 duration-500">
        {children}
    </Link>
}
